...
export default async function Route_Granters_Ungrant(req: Request, res: Response, next: NextFunction) {
    if (!req.granters) {
    ...
    // Grant the challenge
    const grantStatus = await LeaderboardController.ungrant(bodyPayload.data.userUuid, bodyPayload.data.challengeId);
    if (!grantStatus) {
        return Status.send(req, next, {
            status: 500,
